...
def test_queue_stats():
    ...
    print(f"\n{BLUE}{'='*60}{RESET}")
    print(f"{BLUE}1️⃣  TEST: ESTADÍSTICAS DE COLA RQ{RESET}")
...
def test_create_report_with_queue():
    ...
    print(f"\n{BLUE}{'='*60}{RESET}")
    print(f"{BLUE}2️⃣  TEST: CREAR REPORTE CON ENCOLADO ML{RESET}")
    print(f"{BLUE}{'='*60}{RESET}")
    ...
    if response.status_code == 201:
        ...
        print(f"   🖼️  URL: {data['image_url']}")
        print(f"   📍 Ubicación: ({data['latitude']}, {data['longitude']})")
        print(f"   ⚠️  Tipo (placeholder): {data['damage_type']}")
        ...
        if job_id:
            ...
            print(f"\n{YELLOW}⚠️ Reporte creado pero sin job_id (cola no disponible?){RESET}")
...
def test_job_status(token, job_id):
    ...
    print(f"\n{BLUE}{'='*60}{RESET}")
    print(f"{BLUE}3️⃣  TEST: CONSULTAR ESTADO DE JOB{RESET}")
    print(f"{BLUE}{'='*60}{RESET}")
    
    if not job_id:
        print(f"{YELLOW}⚠️ No hay job_id, skip test{RESET}")
        return False
    ...
    if response.status_code == 200:
        ...
        print(f"   ▶️  Iniciado: {job_info.get('started_at', 'N/A')}")
...
def test_wait_for_processing(token, report_id, job_id, max_wait=30):
    ...
    print(f"\n{BLUE}{'='*60}{RESET}")
    print(f"{BLUE}4️⃣  TEST: ESPERAR PROCESAMIENTO (Worker){RESET}")
    print(f"{BLUE}{'='*60}{RESET}")
    
    if not job_id:
        print(f"{YELLOW}⚠️ No hay job_id, verificando solo el reporte...{RESET}")
        ...
        print(f"\n{YELLOW}⚠️ No hay worker activo - test manual requerido{RESET}")
    ...
    print(f"\n\n{YELLOW}⏱️ Timeout esperando procesamiento{RESET}")
...
def test_direct_task_execution():
    ...
    print(f"\n{BLUE}{'='*60}{RESET}")
    print(f"{BLUE}5️⃣  TEST: EJECUCIÓN DIRECTA DE TASK{RESET}")
    print(f"{BLUE}{'='*60}{RESET}")
