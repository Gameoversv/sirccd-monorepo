...
def create_test_image():
    ...
    try:
        ...
        print("⚠️  Pillow no instalado. Crea manualmente test_image.jpg")
...
async def test_create_report_full(client: httpx.AsyncClient, token: str):
    ...
    print("\n" + "="*70)
    print("1️⃣  TEST: CREAR REPORTE COMPLETO")
    print("="*70)
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"      🖼️  URL imagen: {result['image_url']}")
...
async def test_create_report_minimal(client: httpx.AsyncClient, token: str):
    ...
    print("\n" + "="*70)
    print("2️⃣  TEST: CREAR REPORTE MÍNIMO (solo imagen + GPS)")
    print("="*70)
...
async def test_invalid_gps(client: httpx.AsyncClient, token: str):
    ...
    print("\n" + "="*70)
    print("3️⃣  TEST: VALIDACIÓN DE GPS INVÁLIDO")
    print("="*70)
...
async def test_get_report(client: httpx.AsyncClient, token: str, report_id: int):
    ...
    print("\n" + "="*70)
    print(f"4️⃣  TEST: OBTENER REPORTE ID={report_id}")
    print("="*70)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"      🏙️  Ciudad: {result.get('city', 'N/A')}")
...
async def test_unauthorized_access(client: httpx.AsyncClient):
    ...
    print("\n" + "="*70)
    print("5️⃣  TEST: ACCESO SIN AUTENTICACIÓN")
    print("="*70)
