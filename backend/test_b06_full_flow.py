...
# 3. Crear reporte
print("\n3️⃣  CREAR REPORTE (POST /reportes)")
print("-" * 70)
...
if stats['queued'] > 0:
    ...
    try:
        ...
        print(f"   🏷️  Tipo: {result.get('damage_type')}")
        print(f"   ⚠️  Severidad: {result.get('severity')}")
    ...
    print("⚠️  No hay jobs en cola")

# 6. Verificar actualización del reporte
print("\n6️⃣  VERIFICAR REPORTE ACTUALIZADO")
print("-" * 70)
...
try:
    ...
    if response.status_code == 200:
        ...
        print(f"   🏷️  Tipo: {report['damage_type']}")
        print(f"   ⚠️  Severidad: {report['severity']}")
