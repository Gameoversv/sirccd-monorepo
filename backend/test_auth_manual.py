    ...
    try:
        ...
        if response.status_code == 201:
            ...
            if "ya está" in error.get("detail", "").lower():
                print(f"   ⚠️  Usuario ya existe (reutilizando)")
            else:
                print(f"   ⚠️  Error: {error.get('detail')}")
        else:
        ...
        print(f"   ⏱️  Expira en: {login_response['expires_in']} segundos ({login_response['expires_in']//60} minutos)")
        
        # 3. Obtener info del usuario autenticado
        print("\n3️⃣  OBTENER INFO DE USUARIO AUTENTICADO")
        print("   " + "-" * 65)
        ...
        print("\n5️⃣  PROBAR TOKEN INVÁLIDO (debe rechazar)")
        print("   " + "-" * 65)
        ...
        print("\n6️⃣  PROBAR SIN TOKEN (debe rechazar)")
        print("   " + "-" * 65)
        ...
        print("\n7️⃣  PROBAR CONTRASEÑA INCORRECTA (debe rechazar)")
        print("   " + "-" * 65)
        bad_login = {
            "username": "pythonuser",
        ...
        if response.status_code == 200:
            ...
            print(f"   ⚠️  Logout falló (no crítico con JWT stateless)")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("   2. Ver documentación completa: backend/docs/TESTING_B03.md")
        print("   3. Ejecutar tests automatizados: pytest tests/test_auth_integration.py -v")
        sys.exit(0)
    else:
        print("\n⚠️  Algunas pruebas fallaron. Revisar arriba para más detalles.")
        sys.exit(1)
