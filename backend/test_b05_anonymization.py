...
TEST_IMAGE_PATH = Path(__file__).parent / "test_image_with_face.jpg"
TEST_IMAGE_NOFACE_PATH = Path(__file__).parent / "test_image_noface.jpg"
...
async def test_anonymizer_service():
    ...
    print("\n" + "="*70)
    print("1️⃣  TEST: SERVICIO DE ANONIMIZACIÓN (DIRECTO)")
    print("="*70)
    ...
    try:
        ...
        if not TEST_IMAGE_PATH.exists():
            print("   ⚠️  Imagen de prueba no encontrada")
            return
        ...
        print(f"      🌫️  Regiones difuminadas: {stats['regions_blurred']}")
        ...
        output_path = Path(__file__).parent / "test_anonymized_output.jpg"
        ...
        if stats['anonymized']:
            ...
            print("   ℹ️  No se detectaron regiones sensibles (imagen sin cambios)")
...
async def test_report_with_anonymization(client: httpx.AsyncClient, token: str):
    ...
    print("\n" + "="*70)
    print("2️⃣  TEST: CREAR REPORTE CON ANONIMIZACIÓN")
    print("="*70)
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"   ℹ️  La imagen debe estar anonimizada en el storage")
            return result['id']
        else:
...
async def test_report_without_faces(client: httpx.AsyncClient, token: str):
    ...
    print("\n" + "="*70)
    print("3️⃣  TEST: IMAGEN SIN ROSTROS/PLACAS")
    print("="*70)
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"      🆔 ID: {result['id']}")
            print(f"   ℹ️  Imagen no tenía rostros/placas para anonimizar")
        else:
            print(f"   ❌ Error: {response.status_code} - {response.text}")
...
async def test_anonymizer_availability():
    ...
    print("\n" + "="*70)
    print("4️⃣  TEST: DISPONIBILIDAD DE DETECTORES")
    print("="*70)
    ...
    try:
        ...
        if image_anonymizer.plate_cascade is not None:
            ...
            print("   ℹ️  Detector de placas Haar: NO disponible (usando método básico)")
        
        print("\n   ℹ️  Nota: Es normal que el detector de placas Haar no esté disponible.")
        print("   ℹ️  Se usará el método de detección básico por color/forma.")
